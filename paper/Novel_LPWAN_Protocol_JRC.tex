...
\author{
Rachmad Andri Atmoko\textsuperscript{1}, Bayu Sutawijaya\textsuperscript{1*}, Akas Bagus Setiawan\textsuperscript{2},\\
Devasis Pradhan\textsuperscript{3}, Ismail Rakip Karas\textsuperscript{4}\\
...
Email: \textsuperscript{1}ra.atmoko@ub.ac.id, bayu\_sutawijaya@ub.ac.id, \textsuperscript{2}akasbagus\_s@polije.ac.id,\\
...
Fig.~\ref{fig:architecture} illustrates the overall system architecture of the proposed Novel LPWAN protocol. The design shifts protocol complexity from resource-constrained end devices to the gateway, enabling efficient bidirectional communication over duty-cycle-limited links.
...
\end{tikzpicture}
\caption{System architecture of the Novel LPWAN protocol showing device statelessness and gateway intelligence.}
...
Fig.~\ref{fig:bitmap_ack} illustrates the bitmap ACK mechanism. Multiple uplinks are acknowledged in a single downlink frame, reducing gateway-to-device traffic and conserving precious downlink capacity under duty cycle constraints.
...
\end{tikzpicture}
\caption{Bitmap ACK sequence: four uplinks (seq 10--13) acknowledged in a single 7-byte downlink frame with bitmap=0b1111.}
\label{fig:bitmap_ack}
\end{figure}
...
The simulation environment is configured to model realistic LPWAN deployments in the EU868 frequency band. Table~\ref{tab:params} summarizes the key parameters derived from LoRaWAN regional specifications \cite{b41} and empirical studies \cite{b42}. The spreading factor allocation follows the Adaptive Data Rate (ADR) mechanism, where devices closer to the gateway use lower spreading factors (SF7--SF9) for higher throughput, while distant devices use SF10--SF12 for extended range at reduced data rates. The 1\% duty cycle constraint reflects EU regulatory requirements (ETSI EN 300.220), which fundamentally limits uplink capacity and motivates our aggregate ACK design. Simulation duration varies from 24 hours for steady-state analysis to 72 hours for long-term stability evaluation. All experiments use deterministic random seeds (42, 123, 456) to ensure reproducibility across independent runs.
...
Fig.~\ref{fig:topology} shows the simulated network topologies. We evaluate both single-gateway and dual-gateway configurations to assess scalability under varying network densities.
...
% Single Gateway (left)
\node[font=\footnotesize, align=center] at (1.5,3.5) {(a) Single Gateway};
...
\node[font=\tiny] at (1.5,-0.3) {N=10,50,100};

% Dual Gateway (right)
\node[font=\footnotesize, align=center] at (5.5,3.5) {(b) Dual Gateway};
...
\node[font=\tiny] at (5.5,-0.3) {Overlap coverage};

\end{tikzpicture}
\caption{Simulated network topologies: (a) single gateway with radial device distribution, (b) dual gateway with overlapping coverage zones.}
\label{fig:topology}
\end{figure}
